/**
 * Database Utilities - CommonJS Version
 * For use with CommonJS modules (require/module.exports)
 */
...
async function saveAudit(userId, auditData) {
  try {
...
// Get user's audits
async function getUserAudits(userId, limit = 10, offset = 0) {
  try {
    const { data, error } = await supabase
      .from('audits')
      .select('*')
...
async function deleteAudits(auditIds, userId) {
  try {
    const { error } = await supabase
      .from('audits')
      .delete()
    ...
    if (error) throw error;
    return { success: true, count: auditIds.length, error: null };
  } catch (error) {
...
async function checkScanQuota(userId) {
  try {
